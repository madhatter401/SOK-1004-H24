...
df <- df %>%
  ...
  # Filtrerer df til å kun inneholde informasjon fra Norge og endrer variabelnavn
  filter(REF_AREA == "NOR") %>%
  
  # Fjerner kolonner
  select(-REF_AREA, -FREQ, -STATISTICAL_OPERATION, -UNIT_MEASURE, -METHODOLOGY, -DEFINITION, -POVERTY_LINE, -OBS_STATUS, -DECIMALS) %>%
  
  # Endrer navnet på variabler
  rename(
    year = TIME_PERIOD,
    measure = MEASURE,
    age = AGE,
  ...
  # Endrer navnet på GINI og p90/10 for å gjøre det mer forståelig i plottet
  mutate(measure = recode(measure,
                          "INC_DISP_GINI" = "Disponibel inntekt",
                          "INC_MRKT_GINI" = "Markedsinntekt",
                          "INC_GROSS_GINI" = "Bruttoverdi",
                          "D9_1_INC_DISP" = "P90/P10")) %>%

  # Endrer navnet på aldersverdiene for å gjøre det mer forståelig i plottet
  mutate(age = recode(age,
                      "_T" = "Alle aldersgrupper",
                      "Y18T65" = "18 til 65 år",
                      "Y_GT65" = "Over 65 år")) %>%

  #Endrer rekkefølgen på panelene slik at "alle aldersgrupper" kommer først
  mutate(age = factor(age, levels = c("Alle aldersgrupper", "18 til 65 år", "Over 65 år"))) 



# Lager en variabel for et plot for gini-koeffisienten
gini_plot <- df %>%
  
  #Filtrerer alle verdiene i measure som ikke inneholder "p90/p10"
  filter(!grepl("P90/P10", measure)) %>%
  
  #Lager et plot med årstall på x-aksen, koeffisient på y-aksen, forskjellige farger og grupert etter inntektsmål
  ggplot(aes(x = year, y = value, color = measure, group = measure)) +
  
  #Genererer et linjediagram
  geom_line() +
  
  #Legger til punkter på linjediagrammet der de spesifikke ferdiene målt befinner seg
  geom_point() +
  
  #Gjør om plottet til tre kolonner ut i fra "age" med lik verdi på y-aksene
  facet_wrap(~ age, scales = "fixed") +
  
  #lager tittel og navngir aksene
  labs(
    title = "Gini-koeffisient i Norge",
    x = "År",
    y = "Gini-koeffisient",
    color = "Ulikhetsmål") +
  
  #Legger til minimal theme for mindre støy
  theme_minimal() +
  
  #Setter legend til under plottet
  theme(legend.position = "bottom") +
  
  #Setter teksten på x-verdiene til en vinkel på 45 grader for å unngå overlapp
...
#Gjentar operasjonen ovenfor men denne gangen med p90/p10 i stedet
p9010_plot <- df%>%
  filter(grepl("P90/P10", measure)) %>%
  ggplot(aes(x = year, y = value, color = measure, group = measure)) +
  geom_line() +
  ...
  facet_wrap(~ age, scales = "fixed") +
  labs(
    title = "Inntektsulikhet mellom de øverste og nederste 10 % av befolkningen i Norge",
    x = "År",
    y = "x inntekt",
    color = "måleenhet") +
  theme_minimal() +
...
bnp_per_capita <- get_eurostat("nama_10_pc")
```
...
gini_coefficient_22 <- gini_coefficient %>%
  filter(TIME_PERIOD == "2022-01-01" &
           age == "Total" &
           !(geo %in% c("European Union - 27 countries (from 2020)", "Euro area – 20 countries (from 2023)"))) %>% 
  rename("gini" = values) %>% 
  select(geo, gini)

bnp_per_capita_22 <- bnp_per_capita %>%
  filter(TIME_PERIOD == "2022-01-01" &
...
combined_dataset <- livs_kval_22 %>%
  inner_join(gini_coefficient_22, by = "geo") %>%
  inner_join(bnp_per_capita_22, by = "geo")


#OPPGAVE 2b
library(ggrepel)

livskvalitet_gini <- combined_dataset %>%
  ggplot(aes(x = gini, y = livskvalitet)) +
  geom_point(aes(color= geo)) +
  geom_smooth(method = lm, se = FALSE, color = "blue") +
  geom_text_repel(aes(label = geo, color= geo), size = 3) +
  theme_minimal() +
  theme(legend.position = "none") + 
  labs(
    title = "Sammenheng mellom Gini-koeffisient og livskvalitet",
    x = "Gini-koeffisient",
    y = "Livskvalitet",)
print(livskvalitet_gini)

livskvalitet_bnp <- combined_dataset %>%
  ggplot(aes(x = bnp_per_capita, y = livskvalitet)) +
  geom_point(aes(color= geo)) + 
  geom_smooth(method = lm, se = FALSE, color = "blue") +
  geom_text_repel(aes(label = geo, color= geo), size = 3) +
  theme_minimal() +
  theme(legend.position = "none") +
  labs(
    title = "Sammenheng mellom BNP per innbygger og livskvalitet",
    x = "BNP per innbygger",
    y = "Livskvalitet",)
  

print(livskvalitet_bnp)
